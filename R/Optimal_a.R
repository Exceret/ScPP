...
optimal_a = function(sc_dataset, geneList, probs = seq(0.2, 0.45, by = 0.05)){
    ...
    for(i in 1:length(probs)){
        downcells1 = rownames(metadata)[which(metadata$AUCup <= quantile(metadata$AUCup,probs = probs[i]))]
        upcells1 = rownames(metadata)[which(metadata$AUCup >= quantile(metadata$AUCup,probs = (1-probs[i])))]
        downcells2 = rownames(metadata)[which(metadata$AUCdown >= quantile(metadata$AUCdown,probs = (1-probs[i])))]
        upcells2 = rownames(metadata)[which(metadata$AUCdown <= quantile(metadata$AUCdown,probs = probs[i]))]
        ...
        markers <- FindMarkers(sc_dataset, ident.1 = "Phenotype+", ident.2 = "Phenotype-")
        ...
        if(length(genes_pos) > 0 & length(genes_neg) > 0){
            res <- list(Genes_pos = genes_pos,
                        Genes_neg = genes_neg)
            
            library(fgsea)
            fgseaRes <- fgsea(pathways = res, stats = genes_sort)
            NES_dif = fgseaRes$NES[fgseaRes$pathway == "Genes_pos"] - fgseaRes$NES[fgseaRes$pathway == "Genes_neg"]
