...
+ The function `optimal_a()` is used for optimal α selection, containing 3 parameters: sc_dataset, geneList and probs.
  + "sc_dataset" is a seurat object of scRNA-seq data, which can be the output of the function `singcell_Preprocessing()`.
  + "geneList" is a ranked gene list correlated with the phenotypes of interest, which can be the output of functions `marker_Binary()`, `marker_Continuous()` and `marker_Survival()` for binary variables, continuous varaibles and survival data, respectively.
  + "probs" is the α value of ScPP (ranging from 0.2 to 0.45, by 0.05).
...
+ The function `ScPP()` is used for Single Cells’Phenotype Prediction, containing 3 parameters: sc_dataset, geneList and probs.
  ...
  + "probs" is the α value of ScPP ranging from 0.2 to 0.45 (default: 0.2), which is the cutoff for selecting top and bottom α proportion of single cells ranked based on AUCell scores.
...
## **Apply ScPP to select the optimal α**

```R
# Take the binary variable as an example
load(system.file("data/binary.RData",package = "ScPP"))
sc = sc_Preprocess(sc_count)
geneList = marker_Binary(bulk, binary, ref_group = "Normal")
opt = optimal_a(sc, geneList)
opt
```
